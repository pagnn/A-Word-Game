...
def compPlayHand(hand, wordList, n):
    ...
    while True:
        ...
        if word != None:
            score=score+getWordScore(word)
            print word +'earned '+str(getWordScore(word))+' points. Total: '+str(score)+' points'
            handCopy= updateHand(handCopy, word)
            if calculatedHandlen(handCopy) == 0:
    ...
    while True:
        ...
        if flag=='r' or flag=='n':
            if flag=='r':
                ...
                count=count+1
            while True:
                work=raw_input('Enter u to have yourself play, c to have the computer play: ')
                if work == 'u':
